...
export function RegistrationAction({
  ...
  if (registration?.status === "CONFIRMED") {
    return (
      <div className="flex flex-wrap gap-theme-sm">
        <span className={confirmedPillClass}>Registered</span>
        <a className={confirmedPillClass} href="#/registrations">
          View QR
        </a>
      </div>
    );
      ...
      {isPaidWorkshop && isPaymentOpen ? (
        <label className="grid gap-theme-xs text-sm font-bold text-text-primary">
          ...
          <input
            ...
            aria-describedby={
              error ? `payment-error-${workshop.id}` : undefined
            }
          />
        </label>
      ) : null}
  ...
  async function handleRegistration() {
    ...
    if (isPaidWorkshop && !paymentToken.trim()) {
      setError(
        "Enter a payment token before registering for this paid workshop.",
      );
      return;
    }
    ...
    pendingKeyRef.current =
      pendingKeyRef.current ?? createRegistrationIdempotencyKey(workshop.id);
    ...
    try {
      ...
      const storedRegistration = createStoredRegistration({
        ...
        message: qrTicket
          ? undefined
          : "Registration confirmed, but QR retrieval needs a retry.",
      ...
      setMessage(
        qrTicket
          ...
          : "Registration confirmed. Open My QR to retry QR retrieval.",
      );
      setIsPaymentOpen(false);
      setPaymentToken("");
...
const confirmedPillClass = `inline-flex min-h-10 items-center justify-center whitespace-nowrap rounded-full border border-brand-secondary/50 bg-brand-primary/20 px-theme-md text-sm font-extrabold text-brand-secondary shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] transition hover:bg-brand-primary/30 ${focusClass}`;
