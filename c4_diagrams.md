...
Level 2 phân rã UniHub Workshop thành các container đúng với code hiện tại. Backend API là Express service trung tâm; Cron CSV sync chạy trong cùng process với API khi server start; notification delivery chạy bằng BullMQ worker riêng. Redis được dùng cho token-bucket rate limiting, sold-out cache, BullMQ jobs và cache idempotency response; PostgreSQL lưu dữ liệu chính và dùng atomic conditional update để giữ ghế khi đăng ký.
...
System_Boundary(unihub, "UniHub Workshop") {
  ...
  Container(api, "Backend API", "Node.js + Express + TypeScript + pg", "REST API cho workshop, đăng ký, check-in, batch sync; chạy cron CSV sync; gọi AI summary và payment circuit breaker.")
  Container(worker, "Notification Worker", "BullMQ + TypeScript + Gmail SMTP", "Nhận registration.confirmed jobs, gửi email và cập nhật trạng thái notifications.")
  ContainerDb(postgres, "PostgreSQL", "Supabase Postgres + SQL migrations", "Lưu User, Workshop, Registration, Payment, Checkin, Notification và IdempotencyKey.")
  ContainerDb(redis, "Redis", "Redis 7 + ioredis + BullMQ", "Token-bucket rate limit, sold-out marker, cache idempotency response TTL 24h, notification queue.")
...
Rel(api, postgres, "Đọc/ghi bằng pg; atomic decrement khi register", "SQL/TCP")
Rel(api, redis, "Lua token bucket, sold-out marker, idempotency cache, publish notification job", "RESP/TCP")
Rel(worker, redis, "Consume registration.confirmed jobs", "RESP/TCP")
Rel(worker, postgres, "Cập nhật notification delivery status", "SQL/TCP")
...
Luồng này xử lý cả workshop miễn phí và workshop có phí. Điểm quan trọng là request đăng ký đi qua rate limiting, sold-out fast path, idempotency và transaction ngắn với atomic seat decrement. Với workshop có phí, payment gateway được gọi qua circuit breaker để tránh lỗi dây chuyền khi cổng thanh toán không ổn định.
...
Client -> API: POST /api/workshops/{id}/register\nBearer token, Idempotency-Key, paymentToken
